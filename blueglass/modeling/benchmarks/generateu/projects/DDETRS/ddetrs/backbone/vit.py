...
class Attention(nn.Module):
    def __init__(
        ...
        self.rel_pos_h = nn.Embedding(
            rel_sp_dim,
            head_dim,
            _weight=torch.zeros(rel_sp_dim, head_dim, requires_grad=True),
        )
        self.rel_pos_w = nn.Embedding(
            rel_sp_dim,
            head_dim,
            _weight=torch.zeros(rel_sp_dim, head_dim, requires_grad=True),
        )
    ...
    def forward(self, x, H, W, rel_pos_bias=None):
        ...
        attn = calc_rel_pos_spatial(
            ...
            self.window_size,
            self.window_size,
            self.rel_pos_h.weight,
            self.rel_pos_w.weight,
        )
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = x.view(
            -1, self.window_size[1] * self.window_size[0], C
        )  # nW*B, window_size*window_size, C
        B_w = x.shape[0]
        N_w = x.shape[1]
        qkv = (
            self.qkv(x)
            .reshape(B_w, N_w, 3, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
        q, k, v = (
            qkv[0],
        ...
        if use_abs_pos_emb:
            if pos_emb_form == "learnable":
                ...
                self.pos_embed = nn.Embedding(
                    num_patches,
                    embed_dim,
                    _weight=torch.zeros(num_patches, embed_dim, requires_grad=True),
                )
                ...
                self.pos_embed = nn.Embedding(
                    num_patches, embed_dim, _weight=pos_embed
                )  # for fixed sin-cos encoding
