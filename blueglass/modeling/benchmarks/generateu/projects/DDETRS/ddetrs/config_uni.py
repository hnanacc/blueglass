...
def add_ddetrsvluni_config(cfg):
    ...
    cfg.MODEL.DECOUPLE_TGT = (
        False  # detection and grounding use different tgt (nn.Embedding vs Language)
    )
    ...
    cfg.TRAIN_REID_ONLY = (
        False  # whether to train reid only (freeze all other parameters)
    )
    ...
    cfg.BACKBONE_NORM_SAME_LR = (
        False  # backbone norm1, norm2, norm3 use the same lr as base_lr
    )
    ...
    cfg.SOT.FORWARD_ALL_ORIGIN = (
        False  # forward backbone on both the original ref & cur frames
    )
    ...
    cfg.MODEL.DYHEAD.FUSE_CONFIG.DECOUPLE_GAMMA = (
        False  # whether different tasks use different gamma
    )
    ...
    cfg.MODEL.DDETRS.INDEPENDENT_CTRL = (
        False  # use num_dec independent controllers for instance segmentation
    )
    ...
    cfg.TEST.TOPK_FOR_MAPPING = 3  # 15 # change this back for lvis.
    ...
    # harshal.
    cfg.TEST.EVALUATOR_TYPE = (
        "map_standard"  # "map_label_matching", "map_class_agnostic"
    )
    cfg.TEST.LABEL_MATCHING_STRATEGY = "manually_mapped_labels"  # "similarity_basic_prompt", "similarity_synet_prompt", "similarity_learn_prompt", "llm_gpt4_online_prompt", "manually_mapped_labels"
    cfg.TEST.LABEL_MAP_PATH = "weights/label_map.pkl"
