...
from .models.segmentation_condInst_new_encodfpn import (
    DETRsegmVLUni,
...
class DDETRSVLUni(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # we give out raw predictions without any post processing.
        # so that we can control the postprocessing.
        self.use_custom_pred = cfg.TEST.EVALUATOR_TYPE == "custom"
        ...
        self.detr = model_class(
            ...
            lang_as_tgt=cfg.MODEL.LANG_AS_TGT,
        ...
        self.lvis_embed = (
            torch.tensor(np.load(zs_weight_path), dtype=torch.half)
            .permute(1, 0)
            .contiguous()
        )  # D x C
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            output, loss_dict = self.detr.forward(
                images,
                targets,
                self.criterion,
                train=True,
            ...
            output, loss_dict = self.detr.inference(
                images, None, self.criterion, train=False
            )
            ...
            if self.detr.use_iou_branch:
                ...
                iou_pred = [None]
            ## logprobs are not used in inference
    ...
    def language_feature_forward(self, targets):
        ...
        clip_object_descriptions_features = torch.zeros(len(targets), max_len, 768).to(
            self.device
        )
        for ii, target in enumerate(targets):
            text = [
                clip.tokenize(["a " + pred_object])
                for pred_object in target["gt_object_descriptions"]
            ]
            text = torch.stack(text, dim=0).squeeze(1).to(self.device)
            text_features = self.clip_model.encode_text(text)
            text_features = text_features / text_features.norm(dim=1, keepdim=True)
            clip_object_descriptions_features[
                ii : ii + 1, : text_features.size(0), :
            ] = text_features
        ...
        for targets_per_image, batched_input in zip(targets, batched_inputs):
            ...
            for gt_object_description in targets_per_image.gt_object_descriptions.data:
                if ", " in gt_object_description:
                    gt_object_descriptions += random.sample(
                        gt_object_description.split(", "), 1
                    )
                else:
                ...
                new_targets.append(
                    {
                        ...
                        "gt_object_descriptions": gt_object_descriptions,
                    }
                )
            else:
                    ...
                    new_targets.append(
                        {
                            ...
                            "image_size": image_size_xyxy,
                            "gt_object_descriptions": gt_object_descriptions,
                            "image_description": batched_input["image_description"],
                        }
                    )
                else:
                    new_targets.append(
    ...
    def inference(
        ...
        for i, (
            ...
            zip(box_cls, box_pred, pred_object_descriptions, image_sizes, iou_pred)
        ):
            pred_object_des = []
            for ii in range(0, len(output_text), beam_size):
                pred_object_des.append(", ".join(output_text[ii : ii + beam_size]))

            # we return early when doing custom prediction
            # we do the postprocessing later so that we have control.
            if self.use_custom_pred:
                inst = Instances(image_size)
                inst.pred_boxes = Boxes(box_cxcywh_to_xyxy(box_pred_per_image))
                inst.pred_boxes.scale(scale_x=image_size[1], scale_y=image_size[0])
                inst.pred_box_ious = iou_per_image.squeeze().sigmoid()
                inst.pred_box_objectness = logits_per_image.squeeze().sigmoid()
                inst.pred_object_descriptions = ObjDescription(pred_object_des)
                results.append(inst)
                continue
            ...
            if False:  # self.ota:
                ...
                # there is no score thresholding.
                if score_thres > 0.0:
