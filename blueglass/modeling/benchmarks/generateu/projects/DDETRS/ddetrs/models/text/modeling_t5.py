...
from blueglass.configs import FeaturePattern
...
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config, layer_index: int = None):
        super().__init__()
    ...
    def forward(self, hidden_states):
        ...
        if self.is_decoder:
            ...
            intercept_manager().recorder(FeaturePattern.LLM_DECODER_MLP).record(
                layer_name,
                {
                    "pos_img": forwarded_states.detach(),
                },
            )
            forwarded_states = (
                intercept_manager()
                .patcher(FeaturePattern.LLM_DECODER_MLP)
                .patch(f"{layer_name}", forwarded_states)
            )
        ...
        if self.is_decoder:
            assert self.layer_index is not None, "layer_index not set."
            layer_name = f"llm_decoder_resid_denormed_{self.layer_index}"
            intercept_manager().recorder(FeaturePattern.LLM_DECODER_RESID_MLP).record(
                layer_name,
                {
                    "pos_img": hidden_states.detach(),
                },
            )
            hidden_states = (
                intercept_manager()
                .patcher(FeaturePattern.LLM_DECODER_RESID_MLP)
                .patch(f"{layer_name}", hidden_states)
            )
        return hidden_states
...
class T5LayerSelfAttention(nn.Module):
    def __init__(
        self, config, has_relative_attention_bias=False, layer_index: int = None
    ):
        super().__init__()
        self.layer_index = layer_index
        self.is_decoder = config.is_decoder
    ...
    def forward(
        ...
        if self.is_decoder:
            assert self.layer_index is not None, "layer_index not set."
            layer_name = f"layer_{self.layer_index}"
            intercept_manager().recorder(FeaturePattern.LLM_DECODER_MHA).record(
                layer_name,
                {
                    "weights": attention_output[-1].detach(),
                    "outputs": hidden_states,
                },
            )
            hidden_states = (
                intercept_manager()
                .patcher(FeaturePattern.LLM_DECODER_MHA)
                .patch(f"{layer_name}", hidden_states)
            )
        ...
        if output_attentions:
            ...
            return (hidden_states,) + attention_output[1:-1]
...
class T5Block(nn.Module):
    def __init__(
        ...
        self.layer.append(T5LayerFF(config, layer_index=self.layer_index))
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if not use_focal_loss:
                ...
                m = nn.Softmax(dim=-1)
                loss = loss_fct(
                    m(lm_logits.view(-1, lm_logits.size(-1))), labels.view(-1)
                )
    ...
    def get_contrastive_label(
        ...
        object_descriptions,
        output_tokens_id,
        output_tokens_attention_mask,
        box_features,
        negative_description_mask,
    ):
        text_tokens = []
        text_lengths = []
        need_predict = []
        contrastive_label = torch.zeros((box_features.size(0)), dtype=torch.int64).to(
            box_features.device
        )
        exsit_label = []
        text_tokens_for_enc = []
        text_tokens_mask = []
        description_to_exsit_label = (
            []
        )  # torch.zeros((len(object_descriptions)), dtype=torch.int64).to(box_features.device)
        ...
        negative_description_mask = negative_description_mask[
            :, description_to_exsit_label
        ]

        # convert contrastive_label to one-hot
        contrastive_label = (
            torch.zeros(len(contrastive_label), contrastive_label.max() + 1)
            .to(box_features.device)
            .scatter_(1, contrastive_label.unsqueeze(1), 1.0)
        )
        pos_neg_description_mask = negative_description_mask + contrastive_label
        text_tokens_for_enc = torch.cat(text_tokens_for_enc, dim=0).to(
            box_features.device
        )
        ...
        data = {
            "text_tokens_for_enc": text_tokens_for_enc,
    ...
    def img_text_contrastive(self, batch):
        ...
        sims_q2t = torch.matmul(
            object_hidden_states, text_hidden_states.transpose(0, 1)
        )
        ...
        if use_all_negative:
            ...
            loss_itc = F.cross_entropy(
                sims_q2t / self.temp, targets, label_smoothing=0.1
            )

        else:  # use_diff_img_negative:
            loss_itc = torch.zeros(1).to(sims_q2t.device)
            for sim_q2t, label, mask in zip(
                sims_q2t, contrastive_label, pos_neg_description_mask
            ):
                assert sim_q2t.size() == label.size()
                assert mask.size() == label.size()
                loss_itc += F.cross_entropy(
                    sim_q2t.masked_select(mask).unsqueeze(0) / self.temp,
                    label.masked_select(mask).unsqueeze(0),
                    label_smoothing=0.1,
                )
            bs = sims_q2t.size(0)
