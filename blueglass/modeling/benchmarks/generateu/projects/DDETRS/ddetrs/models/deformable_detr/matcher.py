...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        assert (
            cost_class != 0 or cost_bbox != 0 or cost_giou != 0 or cost_mask != 0
        ), "all costs cant be 0"
    ...
    def compute_cost(self, batch_idx, out_bbox, out_prob, targets, nf):
        ...
        gamma = 2.0
        neg_cost_class = (
            (1 - alpha) * (bz_out_prob**gamma) * (-(1 - bz_out_prob + 1e-8).log())
        )
        pos_cost_class = (
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            for target in targets:
                ...
                cost_class[:, idx : idx + num_label] = (
                    pos_cost_class[:, :num_label] - neg_cost_class[:, :num_label]
                )
                idx = idx + num_label
...
class HungarianMatcherVL(nn.Module):
    ...
    def compute_cost(self, batch_idx, out_bbox, out_prob, targets, nf):
        ...
        for idx in range(bz_tgt_ids.size(0)):
            cost_class[:, idx] = (
                pos_cost_class[:, bz_tgt_ids[idx]] - neg_cost_class[:, bz_tgt_ids[idx]]
            ).mean(
                -1
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            for idx in range(tgt_ids.size(0)):
                cost_class[:, idx] = (
                    pos_cost_class[:, tgt_ids[idx]] - neg_cost_class[:, tgt_ids[idx]]
                ).mean(
                    -1
                )  # mean(-1) is to deal with situations where one class name is divided into multiple tokens.
