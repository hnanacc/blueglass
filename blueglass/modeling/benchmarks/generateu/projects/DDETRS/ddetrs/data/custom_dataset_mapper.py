...
class DetrDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        # has to do this here, this is specific to GenU.
        dataset_dict["task"] = "detect"

        if (
            self.is_train and "describe" in dataset_dict["task"]
        ):  # refcoco不进行图像翻转
        ...
        if not self.is_train:
            if "describe" in dataset_dict["task"]:  # this is false.
                new_annotations = []
            ...
            # we would like to keep the annotations during testing.
            # USER: Modify this if you want to keep them for some reason.
            # dataset_dict.pop("annotations", None)
            dataset_dict["test_prompt"] = self.test_prompt
            # return dataset_dict

        if "anno_type" in dataset_dict and dataset_dict["anno_type"] == "image":
            dataset_dict["annotations"] = []

        if "annotations" in dataset_dict:
            # they need an object description which is default to the classname.
            # we need to add them here because we want the dataloading to be compatible
            # with other models and not add more clutter to the anno dicts.

            if len(dataset_dict["annotations"]) > 0:
                object_descriptions = [
                    an["object_description"]
                    for an in dataset_dict["annotations"]
                    if an.get("iscrowd", 0) == 0
                ]
            else:
                object_descriptions = []
