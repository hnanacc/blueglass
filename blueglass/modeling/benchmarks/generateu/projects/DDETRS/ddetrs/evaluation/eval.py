...
class NEWGRiTVGEvaluator(DatasetEvaluator):
    def __init__(self, distributed):  # ,special_token_list
        ...
        self.special_token_list = [
            "<bos>",
        ]  # special_token_list  # tokens in it are not used when score_captions eg. '<bos>'
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            if "instances" in output:
                instances = output["instances"].to(self._cpu_device)
                prediction["instances"] = instances_to_vg_json(
                    instances, input["image_id"], output_logits=False
                )
                h = input["height"]
                w = input["width"]
                ...
                target_boxes = torch.tensor(
                    [target["bbox"] for target in input["targets"]]
                )
                target_text = [
                    target["object_description"] for target in input["targets"]
                ]
            ...
            merged_boxes, merged_text = pluck_boxes(
                merged_ix, target_boxes, target_text
            )

            # === Sort detections by decreasing confidence ====
            sorted_scores, sorted_idx = torch.sort(
                scores, 0, True
            )  # true makes order descending
    ...
    def score_captions(self):
        ...
        for i, record in enumerate(self.records):
            ...
            candidates[i] = [
                " ".join(
                    token
                    for token in record["candidate"].split()
                    if token not in self.special_token_list
                )
            ]

        if len(references) == 0 or len(candidates) == 0:
            return 0.0, [0.0 for _ in range(len(self.records))]
        ...
        if True:  # verbose:
            for k, record in enumerate(self.records):
                if record["iou"] > 0 and record["ok"] == 1 and k % 1000 == 0:
                    ...
                    info_txt = "IOU: {:.3f} OK: {} SCORE: {:.3F} METEOR: {:.3f}".format(
                        record["iou"], record["ok"], scores[k].item(), meteors[k]
                    )
            ...
            for min_meteor in min_meteors:
                ...
                for i in range(n):
                    ...
                    if len(r["references"]) == 0:
                        fp[i] = (
                            1  # nothing aligned to this predicted box in the ground truth
                        )
                    else:
