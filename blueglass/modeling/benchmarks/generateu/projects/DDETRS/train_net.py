...
# Unification
from detectron2.projects.ddetrs.data.custom_dataset_dataloader import (
    build_custom_train_loader,
)
from detectron2.projects.ddetrs.data.custom_dataset_mapper import (
    DetrDatasetMapper,
)  # CustomDatasetMapper

from detectron2.projects.ddetrs import build_detection_test_loader
from blueglass.evaluation import LabelMatchingEvaluator
from blueglass.datasets import register_custom_datasets
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        ), "Doesn't support multiple dataset evaluation yet."

        et = cfg.TEST.EVALUATOR_TYPE
        ds = cfg.DATASETS.TEST[0]

        if False:
            return LabelMatchingEvaluator(ds)
        ...
        if True:
            return LVISEvaluator(
                ...
                True,
                output_folder,
                max_dets_per_image=cfg.TEST.NUM_TEST_QUERIES,
            )

        raise Exception("Invalid evaluator type, set cfg.TEST.EVALUATOR_TYPE")
    ...
    def test_with_TTA(cls, cfg, model):
        logger = setup_blueglass_logger("detectron2.trainer")
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, name="blueglass")
...
def main(args):
    ...
    if comm.is_main_process():
        os.system(
            f"python3 tools/evaluate_ap_fixed.py datasets/lvis/lvis_v1_minival.json  {cfg.OUTPUT_DIR}/inference/lvis_instances_results.json   output/"
        )
    return
...
def new_argument_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="whether to use a unified model for multiple tasks",
    )
    return parser
