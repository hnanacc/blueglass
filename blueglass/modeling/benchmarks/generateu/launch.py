...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--launch",
        type=str,
        default="projects/DDETRS/train_net.py",
        help="Specify launcher script.",
    )
    parser.add_argument(
        "--dist", type=int, default=1, help="Whether start by torch.distributed.launch."
    )
    parser.add_argument(
        "--np", type=int, default=8, help="number of (GPU) processes per node"
    )
    parser.add_argument("--nn", type=int, default=1, help="number of workers")
    parser.add_argument(
        "--port", type=int, default=-1, help="master port for communication"
    )
    ...
    if args.dist >= 1:
        print(
            f"Start {args.launch} by torch.distributed.launch with port {master_port}!",
            flush=True,
        )
