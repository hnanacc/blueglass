...
def pad_to_size(
    tensor: torch.Tensor, size: int, dim: int = 1, padding: int = 50256
) -> torch.Tensor:
    """Pad tensor to size."""
...
def masked_mean(
    values: torch.Tensor, mask: torch.Tensor, axis: bool = None
) -> torch.Tensor:
...
def clip_by_value(
    x: torch.Tensor, tensor_min: float, tensor_max: float
) -> torch.Tensor:
    """
