...
def embedding_bag_triton(
    ...
    trt_out = torch.empty([indices.shape[0], weight.shape[1]], device=indices.device)
...
def embedding_bag_bw_rev_indices(
    ...
    weight_grad = torch.empty_like(weight, device=indices.device)
    per_sample_weights_grad = torch.empty_like(
        per_sample_weights, device=indices.device
    )
    BLOCK_SIZE = 8
