...
_CUSTOM_SPLITS_LVIS = {
    "lvis_v1_minival_genu": ("lvis/", "lvis/lvis_v1_minival.json"),
}
...
for key, (image_root, json_file) in _CUSTOM_SPLITS_LVIS.items():
    register_lvis_instances(
        ...
        (
            os.path.join(f"{DATASET_DIR}", json_file)
            if "://" not in json_file
            else json_file
        ),
        os.path.join(f"{DATASET_DIR}", image_root),
    )
