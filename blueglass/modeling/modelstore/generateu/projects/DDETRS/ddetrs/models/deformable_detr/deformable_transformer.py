...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        ...
        tgt2 = (
            intercept_manager()
            .patcher(FeaturePattern.DET_DECODER_MLP)
            .patch(f"{layer_name}", tgt2)
        )
    ...
    def forward(
        ...
        intercept_manager().recorder(FeaturePattern.DET_DECODER_MHA).record(
            layer_name,
            {"weights": attn_pattern.detach(), "outputs": tgt2.detach()},
        )
        tgt2 = (
            intercept_manager()
            .patcher(FeaturePattern.DET_DECODER_MHA)
            .patch(f"{layer_name}", tgt2)
        )
        ...
        intercept_manager().recorder(FeaturePattern.DET_DECODER_RESID_MHA).record(
            layer_name,
            {"pos_img": tgt.detach(), "refpnts": orig_reference_points.detach()},
        )
        tgt = (
            intercept_manager()
            .patcher(FeaturePattern.DET_DECODER_RESID_MHA)
            .patch(f"{layer_name}", tgt)
        )
        ...
        tgt = (
            intercept_manager()
            .patcher(FeaturePattern.DET_DECODER_RESID_MLP)
            .patch(f"{layer_name}", tgt)
        )
