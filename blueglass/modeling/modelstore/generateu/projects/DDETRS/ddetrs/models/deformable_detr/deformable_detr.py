...
class Generate_with_T5(nn.Module):
    def __init__(
        ...
        self.interp_mode = int(os.environ.get("INTERP_MODE", "0")) == 1
    ...
    def text_decoder(
        ...
        outputs = self.t5_model.generate(
            ...
            output_scores=True,
            output_attentions=self.interp_mode,
            output_hidden_states=self.interp_mode,
        )
        ...
        if num_beams > 1:
            ...
            output_sequences_scores = torch.tensor(sum_top_logprobs).to(
                log_probs.device
            )  # [900]
...
class VL_Align(nn.Module):
    def __init__(self, cfg):
        ...
        self.dot_product_projection_image = nn.Linear(
            cfg.MODEL.DDETRS.HIDDEN_DIM, cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, bias=True
        )  # nn.Identity()
        self.dot_product_projection_text = (
            nn.Identity()
        )  # nn.Linear(cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, cfg.MODEL.DDETRS.HIDDEN_DIM, bias=True) # 768 -> 256
        self.log_scale = nn.Parameter(
            torch.Tensor([cfg.MODEL.DYHEAD.LOG_SCALE]), requires_grad=True
        )
        self.bias_lang = nn.Parameter(
            torch.zeros(cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM), requires_grad=True
        )  # (768，)
...
def _get_clones_list(
    module_list, N, binary_module_for_RPN, binary_module_for_two_stage
):
    new_module_list = nn.ModuleList()
    for module in module_list:
...
class SetCriterion(nn.Module):
    ...
    def loss_labelsVL(self, outputs, targets, indices, num_boxes, log=False, **kwargs):
        ...
        loss_ce = (
            token_sigmoid_binary_focal_loss(
                src_logits, target_classes_onehot, text_mask=ce_mask
            )
            / num_boxes
        )
    ...
    def loss_retrieval_rank(
        self, outputs, targets, indices, num_boxes, log=False, **kwargs
    ):
        """
        ...
        if num_boxes == 0:
            ...
            for batch_idx, (src_idxs, target_idxs) in enumerate(indices):
                ...
                for src_idx, target_idx in zip(src_idxs, target_idxs):
                    ...
                    loss_rank += F.cross_entropy(
                        torch.cat([pos_logit, neg_logit], dim=0).unsqueeze(0),
    ...
    def loss_masks_boxinst(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        image_color_similarity = torch.stack(image_color_similarity_list, dim=0).to(
    ...
    def forward(self, outputs, targets, dataset_source, indices_list):
        ...
        for loss in ["labelsVL", "boxes", "masks"]:  # self.losses:
            ...
            losses.update(
                self.get_loss(
                    loss, outputs, bin_targets, indices_list[-1], num_boxes, **kwargs
                )
            )
        ...
        if "binary_outputs_class" in outputs:
            ...
            l_dict = {k + "_for_test": v for k, v in l_dict.items()}
            losses.update(l_dict)
        return losses
