...
def compare_results(result: List[Frame], result_compare: List[Frame]) -> None:
    ...
    for frame, frame_ref in zip(result, result_compare):
        ...
        if frame.intrinsics is not None:
            ...
            assert frame.intrinsics.focal == pytest.approx(frame_ref.intrinsics.focal)
            assert frame.intrinsics.center == pytest.approx(frame_ref.intrinsics.center)
            assert frame.intrinsics.skew == pytest.approx(frame_ref.intrinsics.skew)
        else:
...
def compare_groups_results(
    ...
    for group, group_ref in zip(result, result_compare):
        ...
        if group.frames is not None:
            ...
            for frame_names, frame_names_ref in zip(group.name, group_ref.name):
