...
def to_nuscenes(dataset: Dataset, mode: str, metadata: Dict[str, bool]) -> DictStrAny:
    ...
    for frame in key_frames:
        ...
        if frame.labels is not None:
            for label in frame.labels:
                ...
                translation = np.dot(sensor2global, label.box3d.location) + np.array(
                    frame.extrinsics.location
                )

                # Using extrinsic rotation here to align with Pytorch3D
                x, y, z, w = R.from_euler("XYZ", label.box3d.orientation).as_quat()
