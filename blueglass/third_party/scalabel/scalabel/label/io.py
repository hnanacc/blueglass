...
def parse(raw_frame: DictStrAny, validate_frames: bool = True) -> Frame:
    ...
    if not validate_frames:
        ...
        if frame.intrinsics is not None:
            frame.intrinsics = Intrinsics.construct(**frame.intrinsics)  # type: ignore
        if frame.extrinsics is not None:
            frame.extrinsics = Extrinsics.construct(**frame.extrinsics)  # type: ignore
        ...
        if frame.labels is not None:
            ...
            for l in frame.labels:
                ...
                if label.box2d is not None:
                    label.box2d = Box2D.construct(**label.box2d)  # type: ignore
                if label.box3d is not None:
                    label.box3d = Box3D.construct(**label.box3d)  # type: ignore
                if label.poly2d is not None:
                    label.poly2d = [
                        Poly2D.construct(**p) for p in label.poly2d  # type: ignore
                    ]
                if label.rle is not None:
                    label.rle = RLE.construct(**label.rle)  # type: ignore
                if label.graph is not None:
                    label.graph = Graph.construct(**label.graph)  # type: ignore
                labels.append(label)
            frame.labels = labels
        return frame
...
def load(inputs: str, nprocs: int = 0, validate_frames: bool = True) -> Dataset:
...
def group_and_sort(inputs: List[Frame]) -> List[List[Frame]]:
    ...
    frames_list = sorted(frames_list, key=lambda frames: str(frames[0].videoName))
    return frames_list
...
def remove_empty_elements(frame: DictStrAny) -> DictStrAny:
    ...
    if isinstance(frame, list):
        return [v for v in (remove_empty_elements(v) for v in frame) if not empty(v)]
