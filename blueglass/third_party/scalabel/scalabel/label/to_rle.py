...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i",
        "--input",
        help=("root directory of scalabel Json files or path to a label " "json file"),
    )
    parser.add_argument(
        "-o",
        "--output",
...
def frame_to_rle(shape: ImageSize, frame: Frame) -> Frame:
    ...
    if has_score:
        labels = sorted(labels, key=lambda label: float(label.score))  # type: ignore
    filt_labels, poly2ds = [], []
    for label in labels:
...
def seg_to_rles(frames: List[Frame], config: Config, nproc: int = NPROC) -> List[Frame]:
