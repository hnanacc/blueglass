...
def acc_single_video_mots(
    ...
    for gt, result in zip(gts, results):
        ...
        for i in range(num_classes):
            ...
            gt_rles_c = [gt_rles[j] for j, gt_ind in enumerate(gt_inds) if gt_ind]
            pred_rles_c = [
                pred_rles[j] for j, pred_ind in enumerate(pred_inds) if pred_ind
            ]
            ...
            if len(gt_rles_c) == 0 and len(pred_rles_c) != 0:
                ...
                distances = 1 - ious_c
                distances = np.where(distances > 1 - iou_thr, np.nan, distances)
...
def evaluate_seg_track(
    ...
    if check_overlap([frame for res in results for frame in res], config, nproc):
        logger.critical(
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--gt", "-g", required=True, help="path to mots ground truth")
    parser.add_argument("--result", "-r", required=True, help="path to mots results")
    parser.add_argument(
        "--config",
        "-c",
        default=None,
