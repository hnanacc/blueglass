...
def check_overlap_frame(frame: Frame, categories: List[str]) -> bool:
    ...
    for label in frame.labels:
        ...
        if overlap_mask is None:
            ...
            if (
                mask_utils.area(mask_utils.merge([overlap_mask, mask], intersect=True))
                != 0.0
            ):
                # found overlap
                return True
            overlap_mask = mask_utils.merge([overlap_mask, mask], intersect=False)
    return False
...
def check_overlap(frames: List[Frame], config: Config, nproc: int = NPROC) -> bool:
    ...
    if nproc > 1:
        ...
        overlaps = [
            check_overlap_frame(frame, category_names) for frame in tqdm(frames)
        ]
...
def handle_inconsistent_length(
    ...
    if len(results) < len(gts):
        ...
        if outlier_results:
            ...
            raise ValueError(
                f"You have videos not in the test set: " f"{str(outlier_results)}"
            )
...
def parse_seg_objects(
    ...
    if any(
        category.isThing is not None and not category.isThing for category in classes
    ):
