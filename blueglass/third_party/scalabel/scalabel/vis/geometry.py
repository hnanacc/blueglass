...
def check_side_of_line(point: NDArrayF64, line: Tuple[NDArrayF64, NDArrayF64]) -> int:
    """Chece which side does a point locate."""
    p1, p2 = line
    det = (point[0] - p1[0]) * (p2[1] - p1[1]) - (point[1] - p1[1]) * (p2[0] - p1[0])
...
class Label3d:
    ...
    def from_box3d(cls, box3d: Box3D) -> Label3d:
        ...
        def rotate(vector: NDArrayF64) -> NDArrayF64:
            if len(box3d.orientation) == 3:
                ...
                rotated = rotate_vector(vector, 0, rot_y + np.pi / 2, 0, center)
            return rotated

        v000 = rotate(center + np.array([-length / 2, -height / 2, -width / 2]))
    ...
    def get_edges_with_visibility(
        ...
        for pair in face_pairs:
            ...
            face2: str = pair[1]
            cw1 = check_clockwise(
                [np.array(v.v2d) for v in faces[face1]["v"] if isinstance(v, Vertex)]
            )
            cw2 = check_clockwise(
                [np.array(v.v2d) for v in faces[face2]["v"] if isinstance(v, Vertex)]
            )
            if cw1 != cw2:
        ...
        for edge, vertices in edges.items():
            if dashed_edges[edge]:
                edges_with_visibility["dashed"].append([v.v2d for v in vertices])
            else:
                edges_with_visibility["solid"].append([v.v2d for v in vertices])
