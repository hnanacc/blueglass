...
class ViewController:
    ...
    def update(self, output: bool = False) -> None:
        ...
        out_path = os.path.join(self.config.out_dir, frame.name.replace(".jpg", ".png"))
        plt.cla()
        self.queue.put(DisplayData(image, frame, self.display_cfg, out_path=out_path))
    ...
    def key_press(self, event: Event) -> None:
        ...
        # Control event.keys for polygon mode
            ...
            self.display_cfg.with_ctrl_points = not self.display_cfg.with_ctrl_points
        elif event.key == "up":
