...
def fetch_image(inputs: Tuple[Frame, str]) -> NDArrayU8:
    ...
    if frame.url is not None and len(frame.url) > 0:
        with urllib.request.urlopen(frame.url, timeout=300) as req:
            image_data = req.read()
        im: NDArrayU8 = np.asarray(Image.open(io.BytesIO(image_data)), dtype=np.uint8)
    else:
...
def gen_3d_cube(
    ...
    * dimension: 3D box size, with length in x direction, height in y direction
    ...
    edges = label3d.get_edges_with_visibility(get_matrix_from_intrinsics(intrinsics))
...
def gen_graph_point(
    ...
    return [mpatches.Circle(node.location, radius=radius, color=color, alpha=alpha)]
