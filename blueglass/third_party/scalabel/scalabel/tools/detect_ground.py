...
GroundOutputType = Union[None, Tuple[NDArrayF64, float, NDArrayF64, NDArrayF64]]
...
def estimate_ground_plane(
    ...
    dist_filtered = np.logical_and(depths < dist_cutoff[1], depths > dist_cutoff[0])
    ...
    for _ in range(iters):
        ...
        if same_plane_points.shape[0] > max_inliers:
            ...
            best_plane_outliers = valid_points[np.logical_not(same_plane_indices)]
...
def main() -> None:
    ...
    parser.add_argument(
        "--min-dist",
        help="Minimum distance of points from " "origin to be considered valid",
        default=3.0,
        type=float,
    )
    parser.add_argument(
        "--max-dist",
        help="Maximum distance of points from " "origin to be considered valid",
        default=25.0,
        type=float,
    )
    parser.add_argument(
        "--min-height",
        help="Minimum height of points, value " "on z-axis to be considered valid",
        default=-2.0,
        type=float,
    )
    parser.add_argument(
        "--max-height",
        help="Maximum height of points, value " "on z-axis to be considered valid",
        default=-1.0,
        type=float,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tracking",
        help="Set flag if tracking. " "Will make all ground planes have same id.",
        action="store_true",
    )
    ...
    expected_normal: NDArrayF64 = np.array(args.expected_normal, dtype=np.float64)

    # Check file extensions
    supported_ext = [".yaml", ".json"]
    if not os.path.splitext(args.bdd_items)[-1] in supported_ext:
        raise ValueError(
            "[Error] Argument --bdd_items should be either" + " a json or a yaml file."
        )
    if not os.path.splitext(args.output)[-1] in supported_ext:
        raise ValueError(
            "[Error] Argument --output should be either" + " a json or a yaml file."
        )
    ...
    if not isinstance(bdd_data, list):
        raise ValueError(
            "[Error] Contents in the json file are not in" + " valid label format."
        )
