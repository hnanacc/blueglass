...
def pairwise_intersection(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    width_height = torch.min(
        boxes1_tensor[:, None, 2:], boxes2_tensor[:, 2:]
    ) - torch.max(
        boxes1_tensor[:, None, :2], boxes2_tensor[:, :2]
    )  # [N,M,2]
...
class BoxMode(IntEnum):
    ...
    @staticmethod
    def convert(
        box: Union[Boxes, _RawBoxType], from_mode: "BoxMode", to_mode: "BoxMode"
    ) -> Union[Boxes, _RawBoxType]:
        """
        Args:
        ...
        if from_mode == BoxMode.XYWHA_ABS and to_mode == BoxMode.XYXY_ABS:
            ...
            arr[:, 2] += arr[:, 0]
            arr[:, 3] += arr[:, 1]
            arr = arr.to(dtype=original_dtype)
        else:
