...
class MrFppi:
    ...
    def __init__(self, result):
        ...
        self.fppi = np.true_divide(
            result.fp, result.nof_imgs
    ...
    def log_avg_mr_reference_implementation(self):
        ...
        xs = np.true_divide(
            self.result.fp, self.result.nof_imgs
        )  # false positives per image
...
class MrFppiMultiPlot:
    def __init__(
        ...
        assert (
            score_method.im_class == MrFppi
        ), "score_method must be a method of class MrFppi"
...
class MultiPlotHelper:
    ...
    def add_plot(
        self, x, y, label, score=None, linestyle=None, color=None, linewidth=None
    ):
        if score is not None:
            label = "{:.3f} - {}".format(score, label)

        self.ax.plot(
            x, y, linestyle=linestyle, label=label, color=color, linewidth=linewidth
        )
        self.ax.legend()
        return self
