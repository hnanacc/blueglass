...
def load_data_ecp(gt_path, det_path, gt_ext=".json", det_ext=".json"):
    ...
    if not gt_files:
        raise ValueError(
            "ERROR: No ground truth files found at given location! ABORT."
    ...
    if not det_files:
        raise ValueError(
            "ERROR: No ground truth files found at given location! ABORT."
            "Given path was: {} and gt ext looked for was {}".format(det_path, det_ext)
        )

    if len(gt_files) != len(det_files):
        raise ValueError(
            "Number of detection json files {} does not match the number of "
            "ground truth json files {}.\n"
            "Please provide for each image in the ground truth set one detection file.".format(
                len(det_files), len(gt_files)
            )
        )
    ...
    for gt_file, det_file in zip(gt_files, det_files):
        ...
        if gt_frame_id != det_frame_id:
            raise ValueError(
                'Error: Frame identifiers do not match: "{}" vs. "{}".'
                "Check number and order of files in"
                " ground truth and detection folder. ABORT.".format(
                    gt_frame_id, det_frame_id
                )
            )
