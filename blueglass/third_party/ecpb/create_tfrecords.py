...
tf.app.flags.DEFINE_string(
    "out_dir", "./data/ecp/tfrecords", "Place to search for the created files."
)
tf.app.flags.DEFINE_string(
    "dataset_name", "ecp-day", "Name of the dataset, used to create the tfrecord files."
)
tf.app.flags.DEFINE_string(
    "anno_path",
    "./data/day/labels",
...
tf.app.flags.DEFINE_string(
    "img_path", "./data/day/img", "Base directory which contains the ecp images."
)
...
tf.app.flags.DEFINE_integer(
    "shuffle", 1, "Shuffle the data before writing it to tfrecord files."
)
...
class ExampleCreator:
    def __init__(self, create_sha256_key=False):
        ...
        self._sess = tf.Session(
            config=tf.ConfigProto(
                device_count={"GPU": 1}, gpu_options={"allow_growth": True}
            )
        )
    ...
    def create_example(self, img_path, anno_path):
        assert (
            ...
            == os.path.splitext(os.path.basename(anno_path))[0]
        )
        ...
        if skipped_annotations > 0:
            logging.debug(
                "Skipped {}/{} annotations for img {}".format(
                    skipped_annotations, len(annotations), img_path
                )
            )
...
def write_shard(args):
    ...
    out_fn = "{}-{}-{:05d}-of-{:05d}".format(
        FLAGS.dataset_name, type, shard, num_shards
    )
    ...
    for cnt, datum in enumerate(data, start=1):
        ...
        example, skipped, sizes, img_size = example_creator.create_example(
            img_path, anno_path
        )
        ...
        if cnt % 10 == 0:
            logging.info(
                "Written {} examples for shard {}-{}/{}".format(
                    cnt, type, shard, num_shards
                )
            )
    ...
    logging.info(
        "Finished shard {}-{}/{}: {} examples written and {} annotations skipped".format(
            type, shard, num_shards, cnt, skipped_annotations
        )
    )
...
def create_jobs(type, data, num_shards, example_creator):
    ...
    shards = [
        data[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)] for i in range(num_shards)
    ]
    ...
    # create and run jobs
    jobs = [
        (shard_id + 1, num_shards, type, data, example_creator)
        for shard_id, data in enumerate(shards)
    ]
    return jobs
...
def process_dataset():
    ...
    if FLAGS.shuffle:
        with open(
            os.path.join(FLAGS.out_dir, FLAGS.dataset_name + "-np_random_state"), "wb"
        ) as f:
    ...
    with ThreadPoolExecutor() as executor:
        result = executor.map(
            write_shard, jobs, chunksize=1
        )  # chunksize=1 is important, since our jobs are long running
    ...
    np.save(
        os.path.join(FLAGS.out_dir, FLAGS.dataset_name + "-img_size_height_width"),
        list(img_sizes)[0],
    )
...
def main(args):
    ...
    logging.info(
        "----- FINISHED in {:02d}:{:02d}:{:02d} -----".format(
