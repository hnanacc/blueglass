...
class ParamsFactory:
    ...
    def ignore_gt(self, gt):
        ...
        if gt["identity"] in self.detections_type:
            ...
            if (
                self.discard_depictions
                and gt["identity"] == "person-group-far-away"
                and "depiction" in gt["tags"]
            ):
                return None
            else:
                return 2
        ...
        if (
            h < self.size_limits[self.difficulty]
            or occlusion >= self.occ_limits[self.difficulty]
            or truncation >= self.occ_limits[self.difficulty]
        ):
    ...
    def preprocess_gt(self, gt):
        if (
            str(gt["identity"]).lower() == "rider"
            and self.rider_boxes_including_vehicles
        ):
    ...
    def skip_det(self, det):
        ...
        too_high = (
            False
            if self.difficulty not in self.size_upper_limits
            else height >= self.size_upper_limits[self.difficulty] * expFilter
        )
