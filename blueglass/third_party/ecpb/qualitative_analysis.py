...
def process_frame(frame, data_path, out_path):
    ...
    fp = [
        det
        for det in dets["children"]
        if det.get("ignore", 0) == 0 and det["matched"] == 0
    ]
...
def process_batch(args):
    ...
    for cnt, frame in enumerate(batch, start=1):
        ...
        if cnt % 10 == 0:
            logging.info(
                "Processed {:3d} images for batch {:2d}/{}".format(
                    cnt, batch_nr, total_batches
                )
            )
    ...
    logging.info(
        "Finished batch {:2d}/{} in {:02d}:{:02d}:{:02d}".format(
            batch_nr, total_batches, elapsed // 3600, (elapsed // 60) % 60, elapsed % 60
        )
    )
...
def run(out_path, pkl_file, data_path, cpu_thread_cnt):
    ...
    os.makedirs(
        out_path
    )  # will throw an exception if path already exists (ensures that things don't get overridden)

    logging.info("Saving results to {}".format(out_path))

    k, m = divmod(len(result), cpu_thread_cnt)
    batches = [
        result[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)]
        for i in range(cpu_thread_cnt)
    ]
    ...
    # create and run jobs
    jobs = [
        (batch_nr, batch, cpu_thread_cnt, data_path, out_path)
        for batch_nr, batch in enumerate(batches, start=1)
    ]

    with ProcessPoolExecutor(cpu_thread_cnt) as executor:
        executor.map(
            process_batch, jobs, chunksize=1
